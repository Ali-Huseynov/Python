...
URL = 'https://www.investaz.az/forex-yarismasi/best-investor-yarisma-neticeleri-1173'

init_html = requests.get(URL).text

init_soup = BeautifulSoup(init_html,'lxml')

comp_links = init_soup.find('div' , class_='ed_row clearfix contest_info_change_input').find('select').find_all('option',limit=24)
...
for num,link in enumerate(comp_links[1:]):
    comp = requests.get(link['value']).text

    soup = BeautifulSoup(comp , 'lxml')

    comp_num, date = get_comp_data(soup)
